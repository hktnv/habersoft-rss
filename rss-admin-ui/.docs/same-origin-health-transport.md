...
`ADMIN_UI_HEALTH_UPSTREAM_ORIGIN` is server-only runtime config. It must be an absolute `http://` or `https://` origin with a non-empty host, optional numeric port, and no userinfo, path, query, fragment, whitespace, shell metacharacters, or production default. A missing or invalid value fails container startup before Nginx serves traffic.
