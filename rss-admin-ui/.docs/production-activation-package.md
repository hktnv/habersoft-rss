...
## MS-027B-R1/R2/R3 Feed Effect Acceptance and Regression Semantics

MS-027B-R1 records `SUCCESS_MS_027B_R1_FEED_ONBOARDING_RECHECK_EFFECT_PRODUCTION_ACCEPTANCE_CLOSED_OPERATOR_REPORTED_EVIDENCE_AUTOMATION_LANDED` by operator report only. `FEED_ONBOARDING_EFFECT_ACCEPTED`, `FEED_RECHECK_EFFECT_ACCEPTED`, `browser-evidence-verify-ok`, and `NGINX_ROUTE_PROOF_ACCEPTED` are accepted for the bounded MS-027B feed onboarding plus recheck effect scope. The durable sanitized receipt lives outside Git under `operator-state/admin-ui-production-activation/ms-027b-r1-feed-onboarding-recheck-effect-accepted-operator-reported-receipt.json`.

MS-027B-R2 records `SUCCESS_MS_027B_R2_EVIDENCE_AUTOMATION_REGRESSION_MODE_LANDED_OPERATOR_RETEST_OPTIONAL`. MS-027B-R1 feed onboarding plus recheck effect production acceptance remains accepted. Do not claim a fresh onboarding effect from an already-present feed regression retest. Regression-mode evidence may classify `FEED_ONBOARDING_PREVIOUSLY_ACCEPTED_NOT_RETESTED`, `FEED_ONBOARDING_ALREADY_PRESENT_REGRESSION_NOT_APPLICABLE`, `FEED_ONBOARDING_ACCEPTANCE_LEDGER_CONTINUITY_OK`, and `RECHECK_EFFECT_ACCEPTED_REGRESSION_OK` only when the prior ledger is tracked; first-time missing onboarding evidence remains `OPERATOR_ACTION_REQUIRED_WITH_REDACTED_REASON`. Local guard: `npm run verify:evidence-regression-mode`.

MS-027B-R3 records `SUCCESS_MS_027B_R3_LIVE_EVIDENCE_REGRESSION_ACCEPTANCE_CLOSED_RECHECK_NOT_RETESTED_EXPECTED` and `MS-027B-R3_LIVE_EVIDENCE_REGRESSION_MODE_ACCEPTED_OPERATOR_REPORTED`. The latest operator-reported evidence preserves the MS-027B-R1 ledger, classifies `FEED_ONBOARDING_PREVIOUSLY_ACCEPTED_NOT_RETESTED`, `FEED_ONBOARDING_ACCEPTANCE_LEDGER_CONTINUITY_OK`, `FEED_RECHECK_NOT_RETESTED_EXPECTED`, `FEED_RECHECK_ACCEPTANCE_LEDGER_CONTINUITY_OK`, `EVIDENCE_REGRESSION_MODE_ACCEPTED`, and `TRUE_EFFECT_CLOSURE_STILL_REQUIRES_FRESH_REDACTED_EVIDENCE`, and does not claim a fresh onboarding or recheck effect. Local guard: `npm run verify:evidence-regression-acceptance`.
