...
MS-026C-R1 durable operator-state receipt outside Git records the automation closeout under `operator-state/admin-ui-production-activation/ms-026c-r1-operator-automation-accepted-feed-recheck-pending-no-target-receipt.json`; MS-027A-R2 durable sanitized receipt records the production promotion/feed-onboarding route-smoke acceptance under `operator-state/admin-ui-production-activation/ms-027a-r2-promotion-feed-onboarding-route-smoke-accepted-operator-reported-receipt.json`; temporary workplace paths are not durable operator artifacts.
...
MS-027A-R1 status is `SUCCESS_MS_027A_R1_PRODUCTION_PROMOTION_IMAGE_FRESHNESS_REMEDIATION_LANDED_OPERATOR_RETEST_REQUIRED`. Production promotion `--apply` now builds/verifies backend and frontend current-HEAD image labels before recreate; `--recreate-only` is restart-only and blocks `backend_image_stale` or `frontend_image_stale` when labels are missing or stale. The operator wrapper also reports `source_not_promoted`, `frontend_route_missing`, `backend_route_missing`, `nginx_template_marker_unresolved`, `auth_not_configured`, `unauthenticated_expected`, `no_eligible_feed_target`, and `accepted_route_smoke_pending_effect`. Local guard: `npm run verify:production-image-freshness`.

MS-027A-R2 status is `SUCCESS_MS_027A_R2_PRODUCTION_PROMOTION_AND_FEED_ONBOARDING_ROUTE_SMOKE_ACCEPTANCE_CLOSED_OPERATOR_REPORTED`. Operator-reported evidence says image freshness accepted, backend runtime image revision matched current HEAD, frontend runtime image revision matched current HEAD, feed onboarding route smoke accepted, authenticated browser evidence accepted, and feed recheck effect remains `PENDING_NO_ELIGIBLE_FEED_RECHECK_TARGET`. Local guard: `npm run verify:production-feed-onboarding-acceptance`.

## MS-027B Feed Onboarding Recheck Effect Flow

MS-027B status is `SUCCESS_MS_027B_FEED_ONBOARDING_RECHECK_EFFECT_FLOW_LANDED_OPERATOR_DEPLOY_RETEST_REQUIRED`. Local fullstack and production-mode RC verify that authenticated feed onboarding creates backend-visible reserved feed state, Operations Drilldown can refresh to a real eligible row, and the bounded recheck route reports the existing due-feed path without raw feed URL or actionRef exposure. Redacted browser evidence and operator automation classify `FEED_ONBOARDING_EFFECT_ACCEPTED`, `FEED_RECHECK_EFFECT_ACCEPTED`, `PENDING_FEED_ONBOARDING_ASYNC_PROCESSING`, `PENDING_NO_ELIGIBLE_FEED_RECHECK_TARGET`, `PENDING_FEED_RECHECK_COOLDOWN`, `FEED_ONBOARDING_REJECTED_SAFE_VALIDATION`, `FEED_RECHECK_ACTION_REJECTED_SAFE_VALIDATION`, and `OPERATOR_ACTION_REQUIRED_WITH_REDACTED_REASON`.

The canonical operator command remains `npm run ops:production:retest`; use `--browser-evidence <redacted-browser-evidence.json>` to classify authenticated/effect evidence without command-line credentials. Operator deploy/retest remains required. Codex did not perform production contact. No production feed was created, seeded, or faked. Validate locally with `npm run verify:feed-onboarding-recheck-effect-flow`.
