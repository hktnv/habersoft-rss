...
const projectName = `habersoft-rss-ms026b-${Date.now()}`;
const uiPort = await freePort();
const apiPort = await freePort();
const adminUsername = "admin";
const adminPassword = "test-only-ms026b-admin-password";
const adminPasswordHash = hashAdminPassword(adminPassword, Buffer.from("ms026b-local-salt-00", "utf8"));
const adminSessionSecret = "test_only_ms026b_admin_session_secret_at_least_32_bytes";

const composeEnv = {
  ...process.env,
  RSS_HABERSOFT_COM_IMAGE: "main-service-app:ms026b-fullstack-local",
  RSS_ADMIN_UI_IMAGE: "rss-admin-ui:ms026b-local",
