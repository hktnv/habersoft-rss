...
- MS-025B adds repository code/docs/tests for the protected read-only operations drilldown API and UI. Its status is `MS-025B_AUTHENTICATED_READ_ONLY_OPERATIONS_DRILLDOWN_READY_NOT_DEPLOYED`; new drilldown production acceptance is pending operator deploy/retest. No production deployment was performed by Codex for MS-025B.
...
- validation: `npm run verify:admin-operations-drilldown`, `npm run test:admin-api-proxy-template`, `npm run test:admin-operations-proxy`, `npm run test:fullstack`, and `npm run test:production-mode-rc`;
- operator boundary: MS-025A-R2 remains accepted for the existing operations summary dashboard. New drilldown production acceptance is pending operator deploy/retest. No production deployment was performed by Codex for MS-025B.

MS-025B operator deploy/retest reminder:

1. Pull main with `git pull --ff-only origin main`.
2. Rebuild/update backend and frontend images as required by current runbooks.
3. Recreate the backend API if the backend API runtime changed.
4. Rebuild/update the configured frontend image if Nginx template or entrypoint source changed.
5. From `rss-admin-ui`, run `npm run ops:compose:recreate` before panel retest.
6. Test `/healthz`, `/status-api/health/live`, `/status-api/health/ready`, unauthenticated `/admin-api/operations/drilldown` returning JSON `401`, authenticated Operations Drilldown JSON/UI data, and logout returning to locked state.
