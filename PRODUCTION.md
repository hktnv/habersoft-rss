...
| `rss-admin-ui` | `MS-026A_BOUNDED_ADMIN_FEED_RECHECK_ACTION_LANDED_OPERATOR_DEPLOY_RETEST_REQUIRED` | MS-024F status/auth shell remains operator-reported accepted; MS-025A-R2 closes read-only operations summary dashboard production acceptance by operator report; MS-025B-R1 closes read-only drilldown production acceptance by operator report; MS-026A feed recheck action is landed locally and still requires operator deploy/retest |
