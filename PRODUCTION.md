...
| `rss-admin-ui` | `MS-026B_OPERATOR_REPORTED_FEED_RECHECK_ROUTE_DEPLOYED_NO_ELIGIBLE_TARGET` | MS-024F status/auth shell remains operator-reported accepted; MS-025A-R2 closes read-only operations summary dashboard production acceptance by operator report; MS-025B-R1 closes read-only drilldown production acceptance by operator report; MS-026A route/proxy/auth/HTML-fallback smoke is operator-reported deployed; feed recheck effect remains pending because production has no eligible feed/actionRef |
