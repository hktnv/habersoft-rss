...
- MS-027A-R1 adds `SUCCESS_MS_027A_R1_PRODUCTION_PROMOTION_IMAGE_FRESHNESS_REMEDIATION_LANDED_OPERATOR_RETEST_REQUIRED`: `npm run ops:production:retest -- --apply` now blocks unless the production checkout matches `origin/main`, backend/frontend `--apply` builds and verifies current-HEAD OCI labels, `--recreate-only` is restart-only and must prove image freshness first, and the operator output can classify `source_not_promoted`, `backend_image_stale`, `frontend_image_stale`, `backend_route_missing`, `frontend_route_missing`, `nginx_template_marker_unresolved`, `auth_not_configured`, `unauthenticated_expected`, `no_eligible_feed_target`, and `accepted_route_smoke_pending_effect`. Validate locally with `npm run verify:production-image-freshness`. Feed onboarding production acceptance remains pending operator retest.
