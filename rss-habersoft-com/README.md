...
MS-022A repository-local source adds disabled-by-default admin auth/session routes for the not-deployed `rss-admin-ui` local full-stack rehearsal: `GET /admin-auth/session`, `POST /admin-auth/login`, and `POST /admin-auth/logout`. Backend admin auth defaults to `ADMIN_UI_AUTH_MODE=disabled`, has no default credential, and requires explicit `single_admin` configuration with a PBKDF2 password hash, session secret, Redis-backed server-side sessions, and an HttpOnly `SameSite=Lax` cookie. MS-022B adds secretless provisioning helpers and production activation validation docs for a future operator-authorized deployment, while the admin UI remains `MS-022B_PRODUCTION_ACTIVATION_PACKAGE_READY - NOT_DEPLOYED`. MS-025A updates the admin session cookie path to `Path=/` so the same opaque HttpOnly cookie authenticates `/admin-auth/*` and `/admin-api/*`; login/logout also clear the historical `Path=/admin-auth` cookie. The backend package version remains `0.1.0-ms-017` because the new admin operations route is protected by admin auth, read-only, local-release-candidate validated, and production deployment/retest remains operator-managed rather than a backend package release.
...
External registry publish, Git tag ve GitHub Release yapilmamistir. Frontend, `rss-panel.habersoft.com`, bagimsiz Agent application ve bagimsiz Tenant applications bu backend status claim'inin kapsami disindadir. MS-025A adds one protected read-only admin route without Prisma schema or migration changes.
