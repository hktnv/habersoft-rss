...
- `main-service-api`: NestJS HTTP API process'i. Health endpoint'lerini, tenant auth/JWKS lifecycle'ini, tenant feed abonelik endpoint'lerini, tenant entry listeleme/detail endpoint'lerini, API-only tenant rate limiting guard'ini ve Agent `X-Agent-Key` auth provider'larini baslatir.
...
MS-008 production `/agent/*` endpoint'i eklemez. `X-Agent-Key` yalniz gelecekteki Agent route'larinda route-level guard olarak kullanilacak altyapidir; Tenant API rotalarini acmaz.
...
Worker servisinde public HTTP portu yayimlanmaz. `docker compose port main-service-worker 3000` port bulunmadigini gostermelidir. `/agent/feeds/due` MS-008'de henuz production route olmadigi icin `404` donmelidir. Worker log'larinda JWKS refresh, tenant auth lifecycle, Agent auth lifecycle, tenant rate-limit capability check, tenant entry list module veya tenant entry detail module baslangici beklenmez.
