...
- `main-service-api`: NestJS HTTP API process'i. Health endpoint'lerini, tenant auth/JWKS lifecycle'ini, tenant feed abonelik endpoint'lerini, tenant entry listeleme/detail endpoint'lerini, API-only tenant rate limiting guard'ini, Agent `X-Agent-Key` auth provider'larini ve `POST /agent/heartbeat` route'unu baslatir.
...
MS-009 itibariyla `X-Agent-Key` ile korunan tek production Agent route'u `POST /agent/heartbeat`tir. Agent key Tenant API rotalarini acmaz.
...
Invoke-WebRequest http://localhost:3000/health/live
Invoke-WebRequest http://localhost:3000/health/ready
Invoke-WebRequest http://localhost:3000/agent/feeds/due
...
Worker servisinde public HTTP portu yayimlanmaz. `docker compose port main-service-worker 3000` port bulunmadigini gostermelidir. `/agent/feeds/due` MS-009'da henuz production route olmadigi icin `404` donmelidir. Worker log'larinda JWKS refresh, tenant auth lifecycle, Agent auth lifecycle, Agent heartbeat module, tenant rate-limit capability check, tenant entry list module veya tenant entry detail module baslangici beklenmez.
